...
FACE_RECOGNITION_TOLERANCE = 0.6  # Increased to be more lenient (0.6 is better than 0.45)
FACE_RECOGNITION_SCALING = 0.25  # Downsample for faster processing
...
class FaceRecognizer:
    def __init__(self, known_faces_dir=KNOWN_FACES_DIR, tolerance=FACE_RECOGNITION_TOLERANCE):
        ...
        self.scaling = FACE_RECOGNITION_SCALING
            ...
            for filename in os.listdir(known_faces_dir):
                if filename.endswith(('.jpg', '.jpeg', '.png')):
                    try:
                        ...
                        small_img = cv2.resize(img, (0, 0), fx=self.scaling, fy=self.scaling)
                        # Try to get face encodings
                        encodings = face_recognition.face_encodings(small_img, model="small")
                        if encodings:
    ...
    def recognize_face(self, face_img):
        """Recognize a face in the given image region with improved robustness"""
        # Skip if face_img is too small
        if face_img.shape[0] < 30 or face_img.shape[1] < 30:
            return "Unknown", False
            
        try:
            # Resize for faster processing
            small_face = cv2.resize(face_img, (0, 0), fx=self.scaling, fy=self.scaling)
            
            # Convert to RGB (face_recognition uses RGB)
            rgb_face = cv2.cvtColor(small_face, cv2.COLOR_BGR2RGB)
            
            # Get face locations first
            face_locations = face_recognition.face_locations(rgb_face, model="small")
            if not face_locations:
                return "Unknown", False
                
            # Get face encodings
            face_encoding = face_recognition.face_encodings(rgb_face, face_locations, model="small")[0]
            ...
            if ENABLE_FACE_RECOGNITION and face_recognizer:
                for face in detected_faces:
                    ...
                    if face_roi.size > 0:  # Make sure ROI is not empty
                        ...
                        if not os.path.exists("debug_faces"):
                            os.makedirs("debug_faces")
                        cv2.imwrite(f"debug_faces/face_{time.time()}.jpg", face_roi)
